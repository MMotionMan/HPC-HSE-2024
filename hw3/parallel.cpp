...
double* compute(double T, double k, double h, double tau, int num_points) {
    ...
    if (rank == 0) {
        ...
        MPI_Recv(part, part_size + 2, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, &status);
    }
    ...
    for (int t = 0; t < steps; ++t) {
        ...
        if (rank < size - 1) {
            MPI_Send(&part_next[part_size], 1, MPI_DOUBLE, rank + 1, 1, MPI_COMM_WORLD);
            MPI_Recv(&part_next[part_size + 1], 1, MPI_DOUBLE, rank + 1, 2, MPI_COMM_WORLD, &status);
        } else {
            part_next[part_size + 1] = 0.0;
        }

        if (rank > 0){
            MPI_Send(&part_next[1], 1, MPI_DOUBLE, rank - 1, 2, MPI_COMM_WORLD);
            MPI_Recv(&part_next[0], 1, MPI_DOUBLE, rank - 1, 1, MPI_COMM_WORLD, &status);
        } else {
            part_next[0] = 0.0;
        }
    ...
    if (rank == 0) {
        for (int i = 0; i < part_size; ++i){
            rod[start_idx + i] = part[i + 1];
        }

        for (int j = 1; j < size; ++j) {
            int offset = j * (num_points / size) + std::min(j, num_points % size);
            int size_to_recv = num_points / size + (j < num_points % size ? 1 : 0);
            MPI_Recv(&rod[offset], size_to_recv, MPI_DOUBLE, j, 0, MPI_COMM_WORLD, &status);
        } 
    } else {
